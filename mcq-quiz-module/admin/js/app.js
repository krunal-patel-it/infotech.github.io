...
const App = {
  ...
  viewDashboard() {
    ...
    const banner = sheetsOn
      ? `<div style="background:#dcfce7;color:#166534;padding:10px 14px;border-radius:8px;margin-bottom:16px;font-size:0.9rem;">
           ☁ ${status}
           <button class="btn btn-sm btn-secondary" style="margin-left:10px;" onclick="App.syncCloudData().then(()=>App.renderView('dashboard'))">Sync now</button>
         </div>`
  ...
  showBulkUpload() {
    const body = `
      <p style="font-size:0.9rem;margin-bottom:12px;color:var(--text-secondary);">
        Upload a <strong>CSV</strong> file (UTF-8). First row must be headers.
      </p>
      <p style="font-size:0.8rem;margin-bottom:12px;background:var(--bg);padding:10px;border-radius:8px;line-height:1.55;">
        <strong>Expected header row (your format):</strong><br>
        Subject, Chapter / Unit, Relevant Session, Q. No., Question, Option A, Option B, Option C, Option D, Right Answer, Explanation, Difficulty, Previous Board Exam?, PYQ Status, PYQ / Source, Year, Source Type, Verification Note<br><br>
        <strong>Right Answer</strong> must be A, B, C or D only.
      </p>
      <div class="form-group">
  ...
  processBulkUpload() {
    ...
    if (!file.name.toLowerCase().endsWith('.csv')) {
      msg.textContent = 'Only CSV files are supported. In Excel: File → Save As → CSV UTF-8.';
      msg.className = 'form-message error';
      return;
    }
    ...
    reader.onload = (e) => {
      try {
        ...
        // Format:
        // Subject | Chapter / Unit | Relevant Session | Q. No. | Question |
        // Option A-D | Right Answer | Explanation | Difficulty |
        // Previous Board Exam? | PYQ Status | PYQ / Source | Year | Source Type | Verification Note
        const colMap = {
          subject: findCol('=subject', 'subject', '=category', 'category'),
          chapterUnit: findCol('chapter / unit', 'chapter/unit', 'chapter', 'unit'),
          relevantSession: findCol('relevant session', '=session', 'session', 'topic'),
          qNo: findCol('q. no', 'q no', 'question no', 'qno'),
          ...
          rightAnswer: findCol('right answer', 'correct answer', 'correct option'),
          ...
          pyqSource: findCol('pyq / source', 'pyq/source', 'pyq source'),
          year: findCol('=year'),
          sourceType: findCol('source type'),
          verificationNote: findCol('verification note', 'verification'),
          questionBank: findCol('question bank'),
          timer: findCol('timer')
        };

        if (colMap.question < 0 || colMap.optionA < 0 || colMap.rightAnswer < 0) {
          msg.textContent = 'Missing required columns: Question, Option A, Right Answer. Found: ' + rawHeaders.join(' | ');
          msg.className = 'form-message error';
          return;
        }
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          const subject = get(colMap.subject) || 'General';
          const chapterUnit = get(colMap.chapterUnit);
          const questionBank = chapterUnit || get(colMap.questionBank) || 'Imported';

          const sourceType = get(colMap.sourceType);
          let verificationNote = get(colMap.verificationNote);
          if (sourceType) {
          ...
          const difficultyRaw = get(colMap.difficulty) || 'Medium';
          const difficulty = /easy/i.test(difficultyRaw) ? 'Easy'
            : /hard|difficult/i.test(difficultyRaw) ? 'Hard'
            : /medium|moderate/i.test(difficultyRaw) ? 'Medium'
            : difficultyRaw;
        ...
        let message = 'Import complete: ' + added + ' questions added, ' + skipped + ' rows skipped.';
        if (skipReasons.length) message += ' (' + skipReasons.join('; ') + ')';
