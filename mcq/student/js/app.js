...
const App = {
  formatDisplayDate(v) {
    ...
    if (/^\d{1,2}:\d{2}/.test(s) && !/GMT/i.test(s)) return s;
    const d = new Date(s);
    if (isNaN(d.getTime())) return s.replace(/\s*GMT[\s\S]*/i, '').replace(/\s*\(.*\)\s*$/, '').trim() || '—';
  ...
  formatDisplayTime(v) {
    if (v == null || v === '') return '';
    const s = String(v).trim();
    if (/^\d{1,2}:\d{2}/.test(s) && !/GMT/i.test(s)) return s.replace(/\s*GMT[\s\S]*/i, '').trim();
    const d = new Date(s);
    if (isNaN(d.getTime())) return s.replace(/\s*GMT[\s\S]*/i, '').replace(/\s*\(.*\)\s*$/, '').trim();
    return d.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', hour12: true });
  },
  formatDisplayDateTime(dateStr, timeStr) {
    const d = this.formatDisplayDate(dateStr);
    const t = this.formatDisplayTime(timeStr);
    if (d === '—' && !t) return '—';
    if (d === '—') return t;
    if (!t) return d;
    return d + ' · ' + t;
  ...
  bindGlobalEvents() {
    ...
    // Register disabled on student panel (admin creates accounts)
  ...
  getHomeHTML() {
    ...
    const prevLogin = user.previousLastLogin || null;
    let lastLoginLine = '';
    if (prevLogin) {
      lastLoginLine = `<p style="margin:6px 0 0;font-size:0.85rem;opacity:0.9;">Last login: <strong>${App.formatDisplayDate(prevLogin)}</strong>${/T|\d{2}:\d{2}/.test(String(prevLogin)) ? ' · ' + App.formatDisplayTime(prevLogin) : ''}</p>`;
    } else {
      lastLoginLine = `<p style="margin:6px 0 0;font-size:0.85rem;opacity:0.9;">This is your first recorded session on this device.</p>`;
    }

    const syncLine = `
      <p style="font-size:0.82rem;margin-bottom:12px;display:flex;flex-wrap:wrap;gap:8px;align-items:center;">
        <span style="color:var(--text-secondary);">Latest Data</span>
        <a href="#" id="link-refresh-data" style="color:var(--primary);text-decoration:underline;font-weight:600;">Refresh Page to Update Data</a>
      </p>`;
    ...
    if (recent.length === 0) {
      ...
      recentHTML = recent.map(a => `
        <div class="recent-item">
          <div class="info">
            <h4>${a.subject || a.category || ''}${a.unit ? ' / ' + a.unit : ''}${a.session && a.session !== 'All' ? ' – ' + a.session : ''}</h4>
            <p>${App.formatDisplayDateTime(a.date, a.time)}</p>
          </div>
    ...
    return `
      ...
      <div class="welcome-card">
        ...
        <p style="margin:4px 0 0;opacity:0.9;">${user.email || ''}</p>
        ${lastLoginLine}
        <p style="margin-top:10px;">Ready to test your knowledge?</p>
      </div>
  ...
  bindHomeEvents() {
    ...
    document.getElementById('link-refresh-data')?.addEventListener('click', async (e) => {
      e.preventDefault();
      const link = e.currentTarget;
      link.textContent = 'Syncing…';
      try {
        if (Quiz.syncFromCloud) await Quiz.syncFromCloud();
        if (typeof API !== 'undefined' && API.isEnabled()) {
          const u = Auth.getCurrentUser();
          if (u && API.getAttempts) {
            const res = await API.getAttempts(u.email);
            if (res && Array.isArray(res.attempts)) {
              // replace local attempts for user
              const all = Storage.get('mcq_attempts') || {};
              all[u.email] = res.attempts;
  ...
  // ---------- SELECT QUIZ ----------
  getSelectQuizHTML() {
    const subjects = Quiz.getSubjects();
    ...
    let forcedTimer = 45;
    let difficultyLocked = !!settings.lockAllDifficulty;
    let forcedDifficulty = settings.defaultDifficulty || 'All';
    ...
    return `
      ...
      <div class="form-card">
        <div class="select-group">
          ...
          <select id="sel-subject" required>
        ...
        <div class="select-group">
          <label>Difficulty ${difficultyLocked ? '<span class="badge" style="background:#fef3c7;color:#b45309;font-size:0.7rem;padding:2px 8px;border-radius:10px;margin-left:6px;">Locked by Admin</span>' : ''}</label>
          <div class="difficulty-options" id="difficulty-options">
            ${difficulties.map((d) => {
              const active = difficultyLocked ? (d === forcedDifficulty) : (d === 'All');
              const disabled = difficultyLocked && d !== forcedDifficulty;
              return `<button type="button" class="option-chip ${active ? 'active' : ''} ${disabled ? 'disabled' : ''}" data-value="${d}" ${disabled ? 'disabled' : ''}>${d}</button>`;
            }).join('')}
          </div>
        </div>
        ...
        <div class="select-group">
          <label>Number of Questions <small id="available-count-label" style="color:var(--text-muted)">(select subject & unit)</small></label>
          <select id="sel-count">
  ...
  bindSelectQuizEvents() {
    ...
    const getDifficulty = () =>
      document.querySelector('#difficulty-options .option-chip.active')?.dataset.value || 'All';
    ...
    const updateAvailableCount = () => {
      ...
      if (!subject || !unit) {
        countSelect.innerHTML = '<option value="">-- Select subject & unit --</option>';
        document.getElementById('available-count-label').textContent = '(select subject & unit)';
        startBtn.disabled = true;
        return;
      }

      const available = Quiz.countAvailable({ subject, unit, session, difficulty });
      ...
      for (let i = 1; i <= available; i++) {
        const selected = i === available ? 'selected' : '';
    ...
    const updateUnits = () => {
      const subject = subjectSel.value;
      if (!subject) {
        unitSel.innerHTML = '<option value="">-- Select Subject first --</option>';
        sessionSel.innerHTML = '<option value="All">All Sessions</option>';
      } else {
        const units = Quiz.getUnits(subject);
    ...
    const updateSessions = () => {
      ...
      if (!subject || !unit) {
        ...
        const sessions = Quiz.getSessions(subject, unit);
        sessionSel.innerHTML = '<option value="All">All Sessions</option>' +
    ...
    subjectSel.addEventListener('change', updateUnits);
    unitSel.addEventListener('change', updateSessions);
    sessionSel.addEventListener('change', updateAvailableCount);

    const settingsNow = Storage.getSettings();
    const diffLocked = !!settingsNow.lockAllDifficulty;
    document.querySelectorAll('#difficulty-options .option-chip').forEach(chip => {
      chip.addEventListener('click', () => {
        if (diffLocked || chip.disabled) return;
    ...
    startBtn.addEventListener('click', () => {
      ...
      const session = sessionSel.value;
      const difficulty = getDifficulty();
      const timer = parseInt(document.querySelector('#timer-options .option-chip.active')?.dataset.value || '45', 10);
      ...
      const result = Quiz.start({ subject, unit, session, difficulty, timerSeconds: timer, count });
      if (!result.success) {
  ...
  handleOptionSelect(btn, isTimeout = false) {
    if (!Quiz.state || Quiz.state.isAnswered) return;
    ...
    document.querySelectorAll('.option-btn').forEach(b => {
      ...
      if (record.correct && letter === record.correct) b.classList.add('correct');
    ...
    if (expBox) {
      ...
      if (expText) expText.textContent = record.explanation || '';
  ...
  finishQuiz() {
    ...
    if (!result) {
      alert('Could not compute quiz result. Please try again.');
      this.showScreen('app');
      this.renderView('home');
      return;
    }
    const user = Auth.getCurrentUser();
    if (user) Quiz.saveResult(user.email, result);
    Quiz.state = null;

    const pct = document.getElementById('result-percentage');
    const att = document.getElementById('result-attempted');
    const cor = document.getElementById('result-correct');
    const wrg = document.getElementById('result-wrong');
    if (pct) pct.textContent = result.percentage + '%';
    if (att) att.textContent = result.attempted ?? result.total;
    if (cor) cor.textContent = result.correct;
    if (wrg) wrg.textContent = result.wrong;
  ...
  getProgressHTML() {
    ...
    const cards = attempts.map(a => `
      <div class="attempt-card">
        <div class="date">${App.formatDisplayDateTime(a.date, a.time)}</div>
        <h4>${a.subject || a.category}${a.unit ? " / " + a.unit : ""}${a.session && a.session !== "All" ? " – " + a.session : ""}</h4>
      ...
      <div class="form-card">
        <div class="profile-info">
          ...
          <div class="profile-row">
            ...
            <span>${App.formatDisplayDate(user.lastLogin)}</span>
          </div>
        </div>
      </div>
