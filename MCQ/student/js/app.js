...
const App = {
  ...
  getHomeHTML() {
    ...
    const syncBanner = sheetsOn
      ? `<p style="font-size:0.8rem;color:var(--success);margin-bottom:12px;">☁ ${API.statusText()}</p>`
      : `<p style="font-size:0.8rem;background:#fef3c7;color:#92400e;padding:8px 12px;border-radius:8px;margin-bottom:12px;">⚠ Local-only mode — progress may not sync to other devices until Google Sheets is connected.</p>`;
