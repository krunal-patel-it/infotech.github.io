...
function ensureUserStub(email, name, mobile) {
  email = String(email || '').trim().toLowerCase();
  if (!email) return;
  if (findUserRow(email)) return;
  const sh = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET_USERS);
  sh.appendRow([
    'u_' + Date.now(),
    email,
    ...
    '', // no password hash – student must re-register or admin sets password
...
function saveAttempt(data) {
  ...
  // Auto-create user row if missing (so Admin always sees the student)
  ensureUserStub(email, data.name, data.mobile);
  sh.appendRow([
    id,
    email,
    data.category || '',
